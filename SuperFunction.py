...
def VectorizedHelperOverInputs(input, robot_x, robot_y, w_current_x, w_current_y, swan_x, swan_y, potential_swan_move_x, potential_swan_move_y, state, current_prob):
    global Constants
    global P
    global Q
    control_x, control_y = Constants.INPUT_SPACE[input]
    ...
    crash_value = a*(1-current_prob)*(1-Constants.SWAN_PROB) + b*(1-current_prob)*(Constants.SWAN_PROB) + c*(current_prob)*(1-Constants.SWAN_PROB) + d*(current_prob)*(Constants.SWAN_PROB)
    if crash_value != 0:
        crash_prob = np.full((Constants.M, Constants.N), crash_value/(Constants.M*Constants.N-1))
        crash_prob[Constants.START_POS[0], Constants.START_POS[1]] = 0    # Reset value where swan starts at start position
        P[state, :, input].reshape((Constants.M, Constants.N, Constants.M, Constants.N), order='F')[Constants.START_POS[0], Constants.START_POS[1],:] += crash_prob
...
vectorized_input_calculator_func = np.vectorize(VectorizedHelperOverInputs, otypes=[bool], excluded=["robot_x", "robot_y", "w_current_x", "w_current_y", "swan_x", "swan_y", "potential_swan_move_x", "potential_swan_move_y", "state", "current_prob"])


def VectorizedHelper(state):
    global Constants
    global P
    global Q
    ...
    # print(potential_swan_move_x, potential_swan_move_y)
    global vectorized_input_calculator_func
    vectorized_input_calculator_func(np.arange(Constants.L),  robot_x, robot_y, w_current_x, w_current_y, swan_x, swan_y, potential_swan_move_x, potential_swan_move_y, state, current_prob)

vectorized_compute_values = np.vectorize(VectorizedHelper, otypes=[bool])

def VectorizedComputeValues(new_constants):
    """
        Computes both transition probabilities and expected stage costs for a given problem
    """
    global Constants
    ...
    Q = np.ones((Constants.K, Constants.L)) * np.inf
    # TODO fill the transition probability matrix P here
    global vectorized_compute_values
    vectorized_compute_values(np.arange(Constants.K))
